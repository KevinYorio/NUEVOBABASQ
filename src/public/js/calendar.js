...
addToClassList() {
    ...
    const isAlreadySelected = Array.from(classList.children).some(item => {
        const existingDateTime = moment(item.textContent.match(/(.*?) -/)[1], 'LLL');
    ...
    if (existingItemWithDate) {
        ...
        listItem.innerHTML = `${this.selectedDateTime.format('LLL')} - ${selectedTime} - <span class="precio-text-color">Precio: $${precio.toFixed(2)}</span>`;

        // Crear el botón de borrado para esta fecha
        let deleteButton = document.createElement('button');
        deleteButton.textContent = 'Borrar';
        deleteButton.classList.add('registro-button2'); // Agrega la clase "registro-button"
        ...
        if (existingItemWithDate) {
            ...
            existingItemWithDate.innerHTML = `${this.selectedDateTime.format('LLL')} - ${selectedTime} - <span class="precio-text-color">Precio: $${precio.toFixed(2)}</span>`;
        } else {
